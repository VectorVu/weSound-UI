import "./item.css";
import React from "react";
import WaveSurfer from "wavesurfer.js";
import { Link } from "react-router-dom";
import request from "../../../api/request";
import useTrack from "../../../hooks/useTrack";


export default function TrackItem({ imageUrl, poster, title, streamUrl, id, changeActiveTrack, activeTrack, playCount }) {
    const { handlePlayTrack } = useTrack();
    const [play, setPlay] = React.useState(false);
    const [wavesurfer, setWaveSuffer] = React.useState();
    const [displayCount, setDisplayCount] = React.useState(playCount);
    let url
    const imgPause = 'data:image/svg+xml;base64,'
    const imgPlay = 'data:image/svg+xml;base64,'
    if (play) {
        url = imgPlay
    } else {
        url = imgPause
    }
    const containerEle = React.useRef();
    React.useEffect(() => {
        if (containerEle.current) {
            const wave = WaveSurfer.create({
                container: containerEle.current,
                barWidth: 2,
                barHeight: .5, // the height of the wave
                progressColor: "#F24908",
                cursorColor: "#FFE9A7",
                barGap: 1,
            });
            wave.load(streamUrl)
            setWaveSuffer(wave);
            wave.on('finish', setPlay(false))
        }
    }, [])
    function onChangeActiveTrack(id) {
        changeActiveTrack(id);
        setPlay(!play);
        handlePlayTrack(wavesurfer)
        // wavesurfer.playPause();
    }
    const addPlayCount = async () => {
        try {
            const count = await request({
                url: `/api/track/playcount/${id}`,
                method: "PUT"
            });
            // setCountTemp()
            setDisplayCount(count.data.playCount);
        } catch (error) {
            console.log(error);
        }
    }
    React.useEffect(() => {
        if (play) {
            addPlayCount()
        }
    }, [play])

    React.useEffect(() => {
        setPlay(id === activeTrack);
        if (id !== activeTrack) {
            if (wavesurfer) {
                wavesurfer.stop();
            }
        }
    }, [activeTrack])

    return (
        <div className="TrackItem" >
            <div className="TrackItem-img">
                <img src={imageUrl}></img>
            </div>
            <div className="TrackItem-content" >
                <div className="TrackItemContent-information">
                    <div className="TrackItemContent-Title">
                        <button className="TrackItemContent-button" onClick={() => onChangeActiveTrack(id)}>
                            <img src={url}></img>
                        </button>
                        <div className="TrackItemContent-containerUserName">
                            <div className="TrackItemContent-userName">{poster}</div>
                            <Link to={`/track/${id}`} style={{ textDecoration: "none", color: "black" }}><div className="TrackItemContent-trackName">{title}</div></Link>
                        </div>
                    </div>
                    <div className="TrackItem-additional">
                        <div className='TrackItemAdditional-likeCount'>
                            <img src='data:image/svg+xml;base64,'></img>
                            <span className='TrackItemAdditionalLikeCount-num'>120</span>
                        </div>
                        <div className='TrackItemAdditional-playCount'>
                            <img src="data:image/svg+xml;base64,"></img>
                            <span className="TrackItemAdditionnalPlayCount-num">{displayCount}</span>
                        </div>
                        <div className='TrackItemAdditional-cmtCount'>
                            <img src='data:image/svg+xml;base64,'></img>
                            <span className="TrackItemAdditionnalCmtCount-num">300</span>
                        </div>
                    </div>
                </div>
                <div className="itemWaveContain" ref={containerEle}>
                </div>
            </div>
        </div>
    )
}